package com.gateway.core.filter.router;

import com.gateway.core.ConfigLoader;
import com.gateway.core.context.ContextStatus;
import com.gateway.core.context.GatewayContext;
import com.gateway.core.filter.Filter;
import com.gateway.core.filter.FilterAspect;
import com.gateway.core.helper.AsyncHttpHelper;
import com.gateway.core.helper.ResponseHelper;
import com.gateway.core.response.GatewayResponse;
import com.netflix.hystrix.*;
import com.netflix.hystrix.exception.HystrixTimeoutException;
import com.netflix.hystrix.strategy.properties.HystrixPropertiesFactory;
import com.gateway.common.config.Rule;
import com.gateway.common.enums.ResponseCode;
import com.gateway.common.exception.ConnectException;
...
import static com.gateway.common.constant.FilterConst.*;
...
public class RouterFilter implements Filter {
	...
	private class RouterHystrixCommand extends HystrixCommand<Object> {
		...
		protected Object run() throws Exception {
			// 实际路由操作
			// 注意：此处route方法中发起请求为异步操作，很大概率无法触发熔断超时，故需要给hystrix配置异步机制
//			route(context, config).get();
			Request request = context.getRequest().build();
			CompletableFuture<Response> future = AsyncHttpHelper.getInstance().executeRequest(request);
			Response response = future.get();
			complete(request, response, null, context);
			return null;
		}
