package com.gateway.core.filter.flow;

import com.alibaba.fastjson.JSON;
import com.gateway.common.config.Rule;
import com.gateway.common.constant.FilterConst;
import com.gateway.common.utils.redis.JedisUtil;
import com.gateway.core.filter.flow.algorithm.SlidingWindowAlgorithm;
import com.gateway.core.filter.flow.algorithm.StableAlgorithm;
import com.gateway.core.filter.flow.algorithm.VoteBucketAlgorithm;
import org.apache.commons.lang3.StringUtils;

import java.util.Map;
import java.util.concurrent.ConcurrentHashMap;

import static com.gateway.common.constant.FilterConst.FLOW_CTL_LIMIT_DURATION;
import static com.gateway.common.constant.FilterConst.FLOW_CTL_LIMIT_PERMITS;
	...
	public void doFlowControlFilter(Rule.FlowControlConfig flowControlConfig, String serviceId) {
		...
		if (FilterConst.FLOW_CTL_MODE_DISTRIBUTED.equalsIgnoreCase(flowControlConfig.getMode())) {
			flag = switch (flowControlConfig.getAlgorithm()) {
				case FlowAlgorithmConstant.VOTE_BUCKET_ALGORITHM ->
						new VoteBucketAlgorithm(new JedisUtil()).executeResp(flowControlConfig, key);
				case FlowAlgorithmConstant.FIXED_WINDOWS_ALGORITHM ->
						new StableAlgorithm(new JedisUtil()).executeResp(flowControlConfig, key);
				case FlowAlgorithmConstant.MOVE_WINDOWS_ALGORITHM ->
